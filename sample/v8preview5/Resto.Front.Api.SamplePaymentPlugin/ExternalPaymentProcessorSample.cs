    ...
    internal sealed class ExternalPaymentProcessorSample : IPaymentProcessor, IDisposable
    {
        ...
        public ExternalPaymentProcessorSample()
        {
            subscriptions = new CompositeDisposable
            {
                ...
                PluginContext.Notifications.CafeSessionOpening.Subscribe(x => CafeSessionOpening(x.printer)),
        ...
        public void ReturnPaymentWithoutOrder(
            ...
            [NotNull] IViewManager viewManager)
        {
            // if you need to implement return payment without Syrve order, you should register your IExternalPaymentProcessor as 'canProcessPaymentReturnWithoutOrder = true'
        ...
        public bool CanPaySilently(
            ...
            IPaymentDataContext context)
        {
            PluginContext.Log.Info($"Checking of payment silent processing {sum}, order id {orderId}.");

            var customData = context.GetCustomData<PaymentAdditionalData>();
            return customData?.SilentPay ?? false;
        }

        public void OnPaymentDeleting([NotNull] IOrder order, [NotNull] IPaymentItem paymentItem, [NotNull] IUser cashier, IReceiptPrinter printer,
            [NotNull] IViewManager viewManager, IPaymentDataContext context)
        {
            ...
            var showPaymentFailedError = true;
        ...
        private void CafeSessionOpening([NotNull] IReceiptPrinter printer)
        {
            ...
            const string message = "SamplePaymentPlugin: 'I can not connect to my server to open operation session. But I'll not stop openning Syrve POS cafe session.'";
