...
namespace Resto.Front.Api.SamplePlugin.PreliminaryOrders
{
    internal sealed partial class OrderEditView
    {
        ...
        private void LstItemsSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            // The price category of the branch where the default table is located
            var priceCategory = PluginContext.Operations.GetHostTerminalsGroup().PriceCategory;

            var groupModifiers = dish.GetGroupModifiers(priceCategory);
            var simpleModifiers = dish.GetSimpleModifiers(priceCategory);
