...
namespace Resto.Front.Api.SamplePlugin
{
    internal sealed class EditorTester : IDisposable
    {
        ...
        private void AddOrderSelectionCheckBox()
        {
            ...
            {
                Content = "Apply changes to order that is opened in Syrve POST",
        ...
        /// <summary>
        /// Receiving and printing X-Report.
        /// </summary>   
        ...
        /// <summary>
        /// Creating an order for a specific waiter with adding of the guest Alex and the product from the assortment.
        /// </summary>   
        private void CreateOrderToWaiter()
        {
            ...
            if (!ChooseItemDialogHelper.ShowDialog(PluginContext.Operations.GetUsers(), user => user.Name, out var selectedUser, "Select waiter", window))
                return;
            var newOrder = editSession.CreateOrder(null, waiter: selectedUser); // the order will be created on the table by default
            editSession.ChangeOrderOriginName("Sample Plugin", newOrder);
        ...
        {
            var credentials = os.GetCredentials();
            var client = os.SearchClients(credentials, "Jason").Last();
        ...
        private static void AddProductComment([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            ...
            os.ChangeOrderItemComment("Cook without salt.", order, orderItem, os.GetCredentials());
        }
        ...
        /// <summary>
        /// Changing the dishes cooking location.
        /// </summary>
        private static void ChangeCookingItemsCookingPlace([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            ...
            var restaurantSection = os.GetTerminalsGroupRestaurantSections(os.GetHostTerminalsGroup()).Single(rs => rs.Name == "Bar");
