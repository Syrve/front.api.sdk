    ...
    internal sealed class ButtonsTester : IDisposable
    {
        ...
        public ButtonsTester()
        {
            subscriptions = new CompositeDisposable
            {
                ...
                Operations.AddButtonToPluginsMenu("SamplePlugin: Open Syrve.com", _ => OpenSyrveSite()),
                ...
                }, SyrveIcon).buttonRegistration,

                Operations.AddButtonToPluginsMenu("Navigate to order", x =>
                {
                    var order = Operations.GetOrders().LastOrDefault(o => o.Status == OrderStatus.New);
                    if (order == null)
                        return;
                    x.vm.NavigateToOrderAfterOperation(order);
                }),
                Operations.AddButtonToClosedOrderScreen("Navigate to order", x =>
                {
                    var order = Operations.GetOrders().LastOrDefault(o => o.Status == OrderStatus.New);
                    if (order == null)
                        return;
                ...
                Operations.AddButtonToProductsReturnScreen("Navigate to order", x =>
                {
                    var order = Operations.GetOrders().LastOrDefault(o => o.Status == OrderStatus.New);
                    if (order == null)
                        return;
                ...
                Operations.AddButtonToOrderEditScreen("Navigate to current order", x =>
                {
                    // Navigation into itself is redundant and doesn't happen.
                    x.vm.NavigateToOrderAfterOperation(x.order);
                }, SyrveIcon),
                Operations.AddButtonToOrderEditScreen("Navigate to other order", x =>
                {
                    var otherOrder = Operations.GetOrders().LastOrDefault(o => o.Status == OrderStatus.New && o.Id != x.order.Id);
                    if (otherOrder == null)
                        return;

                    x.vm.NavigateToOrderAfterOperation(otherOrder);
                }, SyrveIcon),
            };
        }
        ...
        private static void OpenSyrveSite()
        {
            using (var process = new Process
            {
                EnableRaisingEvents = false,
                StartInfo = { FileName = @"https://en.syrve.help/articles/#!api/getting-started-api" }
            })
            {
                process.Start();
            }
