...
namespace Resto.Front.Api.SamplePlugin
{
    internal sealed class OrderStatusChangeNotifier : IDisposable
    {
        ...
        public OrderStatusChangeNotifier()
        {
            ...
            var orders = PluginContext.Operations.GetOrders(true);
            // Now remote orders are placed in the API
            var deletedAndStornedOrderIds = orders
                .Where(o => o.Status == OrderStatus.Deleted)
                .Select(o => o.Id)
                .ToList();
            // deleting empty orders is possible on a non-cash terminal as well
            subscriptions.Push(
                PluginContext.Notifications.BeforeDeleteOrder.Subscribe(x =>
                {
        ...
        {
            return terminal.Id == order.LastChangedTerminalId;
        }

        private bool IsStornedOnCurrentTerminal(IOrder order)
        {
            // Using this exotic method, we additionally filter remote orders from returned ones.
